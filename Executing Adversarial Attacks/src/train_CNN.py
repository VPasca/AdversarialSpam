...
def evaluate_cnn(model, batch, sentence_len):
    ...
    ProbFile = open("ProbsCnn.txt","a") 
    LabelFile = open("TrecLabelsCnn.txt","a") 
    PredFile = open("CnnPred.txt","a") 
    LabelFile.write(str(batch.label.data.cpu().numpy()))
    ProbFile.write(str(preds.data.cpu().numpy()))

    eval_acc=sum([1 if preds.data.cpu().numpy()[i][j]>0.5 else 0 for i,j in enumerate(batch.label.data.cpu().numpy()[0])])

    #if preds.data.cpu().numpy()[i][j]
    ([PredFile.write("REAL ") if preds.data.cpu().numpy()[i][0]>0.5 else PredFile.write("FAKE ") for i,j in enumerate(batch.label.data.cpu().numpy()[0])])
...
def main():
    ...
    for epoch in range(5):
        print("epoch ", epoch)
        LabelFile = open("TrecLabelsCnn.txt","a")
        AccFile = open("AccCnn.txt","a")
        ProbFile = open("ProbsCnn.txt","a")
        PredFile = open("CnnPred.txt","a") 

        EpochString = "Epoch"
        #if epoch == 0: EpochString + "ONE"
        #if epoch == 1: EpochString + "TWO"
        #if epoch == 2: EpochString + "THREE"
        #if epoch == 3: EpochString + "FOUR"
        #if epoch == 4: EpochString + "FIVE"

        LabelFile.write(EpochString)
        ProbFile.write(EpochString)
        PredFile.write(EpochString)
        ...
        AccFile.write(str(test_acc/len(test)))
        ...
        for batch in train_iter:
            mycount+=1
            print(epoch,(test_acc/len(test)),mycount,len(train_iter))
            #total_n+=16
            optimizer.zero_grad()
