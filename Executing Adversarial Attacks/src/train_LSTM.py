...
def evaluate(model, batch):
    ...
    print("this is batch label")
    print(batch.label)

    LSTMProbFile = open("LSTMProb.txt","a") 
    LSTMLabelFile = open("LSTMLabel.txt","a") 
    LSTMPredFile = open("LSTMPred.txt","a") 
    LSTMLabelFile.write(str(batch.label.data.cpu().numpy()))
    LSTMProbFile.write(str(preds.data.cpu().numpy()))
    ...
    ([LSTMPredFile.write("REAL ") if preds.data.cpu().numpy()[i][0]>-0.693147181 else LSTMPredFile.write("FAKE ") for i,j in enumerate(batch.label.data.cpu().numpy()[0])])


    print("this is eval acc" )
    print(eval_acc)
...
def main():
    ...
    for epoch in range(5):
        ...
        LSTMAccFile = open("LSTMAcc.txt", "a")
        LSTMProbFile = open("LSTMProb.txt", "a")
        LSTMLabelFile = open("LSTMLabel.txt", "a")
        LSTMPredFile = open("LSTMPred.txt", "a")

        EpochString = "Epoch "
        LSTMAccFile.write(EpochString)
        LSTMProbFile.write(EpochString)
        LSTMLabelFile.write(EpochString)
        LSTMPredFile.write(EpochString)
	

        for batch in test_iter:
            mycount+=1
            print(test_acc,mycount,len(test_iter))
            test_acc+=evaluate(classifier, batch)
        print('Test accuracy: {0}'.format(test_acc/len(test)))
        LSTMAccFile.write(str(test_acc/len(test)))
        running_loss = 0.0
        mycount=0
        for batch in train_iter:
            mycount+=1
            print(mycount,len(train_iter))
            ...
            if step % opt.log_every == 0:
                ...
                running_loss = 0.0
        torch.save(classifier, os.path.join("lstm_model_{0}".format(epoch + 1)))
